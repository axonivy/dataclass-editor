...
export const useKnownHotkeys = () => {
  ...
  const openForm = useMemo<KnownHotkey>(() => {
    const hotkey = 'F';
    return { hotkey, label: t('hotkey.openForm', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);
  ...
  const addAttr = useMemo<KnownHotkey>(() => {
    const hotkey = 'A';
    return { hotkey, label: t('hotkey.addAttribute', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);

  const combineAttr = useMemo<KnownHotkey>(() => {
    const hotkey = 'C';
    return { hotkey, label: t('hotkey.combineAttribute', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);

  const deleteAttr = useMemo<KnownHotkey>(() => {
    const hotkey = 'Delete';
    return { hotkey, label: t('hotkey.deleteAttribute', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);
